...
DEFAULT_STOCKFISH_PATH = os.path.join(os.path.dirname(__file__), "stockfish", "stockfish.exe")
...
def analyze_game():
    try:
        ...
        if not STOCKFISH_PATH or not os.path.exists(STOCKFISH_PATH):
            return jsonify({
                "error": (
                    "Stockfish engine not found. Set STOCKFISH_PATH to your Stockfish executable "
                    "or place it at backend/stockfish/stockfish.exe."
                )
            }), 500
